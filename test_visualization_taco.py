...
def extract_brainco(ep_path: str, urdf_path: str,
    ...
    skeleton = sample["skeleton_poses"][frame_idx].cpu().numpy()
    wrist_p = sample["wrist_poses"][frame_idx].cpu().numpy()
    ...
    return dict(
        ...
        fp_left=fp_left,
        fp_right=fp_right,
        fp_sizes_l=fp_sizes_l,
        fp_sizes_r=fp_sizes_r,
        wrist_poses=wrist_p,
...
def _extract_from_pkl_dataset(data_root: str, seq_idx: int, frame_idx: int, label: str) -> dict:
    ...
    print(
        f"  {label}: total_pkls={len(all_pkls)}, pkl_idx={pkl_idx}({all_pkls[pkl_idx].name}), "
        f"window={idx}, frame_idx={frame_idx}"
    )

    skeleton = sample["sensor_poses"][frame_idx].cpu().numpy()
    wrist_p = sample["wrist_poses"][frame_idx].cpu().numpy()
    tac = sample["sensor"][frame_idx].cpu().numpy()
...
def extract_taco(data_root: str, seq_idx: int = -1, frame_idx: int = 0) -> dict:
    return _extract_from_pkl_dataset(data_root, seq_idx, frame_idx, label="TACO")
...
def _setup_axes(axes_3d, axes_2d, title,
    ...
    for ax, (_, _, (xl, yl)) in zip(axes_2d, PLANES):
        ...
        ax.set_ylabel(yl)
    axes_3d.cla()
    axes_3d.set_xlim(*lim3d)
    axes_3d.set_ylim(*lim3d)
    axes_3d.set_zlim(*lim3d)
...
def _draw_hand_skeleton(axes_3d, axes_2d, sk, fp, fp_sz, color, sk_lines):
    ...
    axes_3d.scatter(sk[:, 0], sk[:, 1], sk[:, 2], c=color, s=_JOINT_PT_SIZE_3D, marker="o", alpha=0.8, zorder=4)
    for ax, (_, (i0, i1), _) in zip(axes_2d, PLANES):
        ax.scatter(sk[:, i0], sk[:, i1], c=color, s=_JOINT_PT_SIZE_2D, marker="o", alpha=0.8, zorder=4)
...
def visualize_local(axes_3d, axes_2d, data: dict):
    offsets = [np.array([-0.12, 0.0, 0.0], np.float32),
               np.array([0.12, 0.0, 0.0], np.float32)]
    quiver_len = 0.04
    ...
    for (sk_raw, fp_raw, fp_sz, color, wrist_color, label), off in zip(
        ...
        _draw_wrist_axes(axes_3d, axes_2d, off, np.eye(3, dtype=np.float32), wrist_color, label, quiver_len)
...
def visualize_global(axes_3d, axes_2d, data: dict):
    ...
    wrist_pos = data["wrist_poses"][:, :3].astype(np.float32)
    print(f"\n=== visualize_global ===")
    print(f"  {data['info']}")
    for side, idx in [("Left ", 0), ("Right", 1)]:
        ...
        print(
            f"  {side}  pos(xyz) = [{pos[0]:+.4f}, {pos[1]:+.4f}, {pos[2]:+.4f}]"
            f"  rot6d = [{', '.join(f'{v:+.4f}' for v in r6d)}]"
        )
    wrist_R = rot6d_to_mat(data["wrist_poses"][:, 3:].astype(np.float32))
            ...
            for ai, ac in enumerate(_RAW_COLORS):
                ...
                for ax, (_, (i0, i1), _) in zip(axes_2d, PLANES):
                    ax.annotate("", xy=(end[i0], end[i1]), xytext=(pos[i0], pos[i1]),
                                arrowprops=dict(arrowstyle="->", color=ac, lw=2.0, linestyle="dashed"))
...
def _render_and_save(bc, taco, ok, out_path: str):
    ...
    fig.suptitle(
        "BrainCo / TACO / OakInkV2  -  Local (rows 0-2) / Global vroot (rows 3-5)",
        fontsize=11,
    )
    ...
    visualize_local(*make_row(1), taco)
    ...
    visualize_global(*make_row(4), taco)
...
def main():
    ...
    parser.add_argument("--brainco_ep", default="pretraining_dataset/brainco/pot/episode_0000")
    ...
    parser.add_argument("--taco_root", default="pretraining_dataset/TACO")
    parser.add_argument("--taco_seq", type=int, default=3, help="TACO pkl file index (-1=middle)")
    parser.add_argument("--taco_frame", type=int, default=40, help="TACO frame within window")
    ...
    parser.add_argument("--out", default="vis/compare_taco.png")
    ...
    _ds = BraincoSSLDataset(
        config=config,
        data_path=args.brainco_ep,
        brainco_urdf_path=args.brainco_urdf,
        joint_poses=True,
    )
    ...
    print("Loading TACO sample...")
    taco = extract_taco(args.taco_root, seq_idx=args.taco_seq, frame_idx=args.taco_frame)

    print("Loading OakInkV2 sample...")
    ok = extract_oakinkv2(args.oakink_root, seq_idx=args.oakink_seq, frame_idx=args.oakink_frame)

    sample_indices = range(0, total_samples, args.frame_step)
    for sample_idx in sample_indices:
        print(f"\n--- sample_idx={sample_idx} ---")
        bc = extract_brainco(args.brainco_ep, args.brainco_urdf, sample_idx=sample_idx, frame_idx=args.brainco_frame)
        stem = out_path.stem
        suffix = out_path.suffix
        out_file = out_path.parent / f"{stem}_sample{sample_idx:04d}{suffix}"
        _render_and_save(bc, taco, ok, str(out_file))
