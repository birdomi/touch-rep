...
def visualize_global(axes_3d, axes_2d, data: dict):
    ...
    data["sk_left"][:, 0]  = -data["sk_left"][:, 0]
    data["fp_left"][:, 0]  = -data["fp_left"][:, 0]
    sk_left_g  = wrist_pos[0] + (wrist_R[0] @ data["sk_left"].T).T
    sk_right_g = wrist_pos[1] + (wrist_R[1] @ data["sk_right"].T).T
    fp_left_g  = wrist_pos[0] + (wrist_R[0] @ data["fp_left"].T).T
...
def main():
    ...
    # parser.add_argument("--brainco_ep",     default="dataset/brainco/pretraining/towel/episode_0000")
    parser.add_argument("--brainco_ep",     default="dataset/brainco/downstream/grasp_detection/grasp_success/episode_0001")
    ...
    parser.add_argument("--frame_step",     type=int, default=50, help="sample index step for multi-frame output")
