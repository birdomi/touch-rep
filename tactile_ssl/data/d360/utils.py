...
"""
Module with utilities for loading and processing D360 multisensory data.
This module provides functions to load audio, image, pressure and IMU data
from D360 datasets, as well as functions to preprocess and synchronize this data.
"""
...
def load_ros_data(
    ...
    filter_devices: Optional[List[int]] = None,
):
    ...
    with open(file, "rb") as f:
        ...
        topics = sorted(
            [
                channel.topic
                for _, channel in reader.get_summary().channels.items()
                if "d360" in channel.topic and check(channel.topic)
            ]
        )

    devices = sorted(list(set([topic.split("/", 2)[1] for topic in topics])))
    if filter_devices is not None:
        assert len(filter_devices) < len(devices), "Invalid device id to remove"
        for filter_id in filter_devices:
            devices.pop(filter_id)
    topics = [topic for topic in topics if topic.split("/", 2)[1] in devices]
...
def process_d360_rosmsgs(devices, topics, raw_msgs, reference_timestamp: float = 0.0):
    ...
    for topic in topics:
        ...
        if topic in raw_msgs[device]:
            if "image_raw/compressed" in topic:
                stamp = [get_timestamp(msg) - reference_timestamp for msg in raw_msgs[device][topic]]
...
def save_to_pickle(
    ...
    devices, topics, raw_msgs = load_ros_data(file, image=image, audio=audio, imu=imu, pressure=pressure)
    data = process_d360_rosmsgs(devices, topics, raw_msgs)
    pd.to_pickle(data, f"{file.rsplit('.', 1)[0]}.pickle")
