...
def draw_2d_axes(
    ...
    """
    Draw 2D axes in 2D matplotlib plot.
    Args:
        ax: matplotlib 2D axis
        world_T_camera: n x 3 x 3 transformation matrix from camera to world frame
    """

    origin = np.array([0, 0])
    x_axis = np.array([axis_length, 0])
    y_axis = np.array([0, axis_length])

    if world_T_camera is not None:
        origin = world_T_camera[:, :2, 2]
        x_axis = world_T_camera[:, :2, 0]
        y_axis = world_T_camera[:, :2, 1]

    # Plot the axes
    ax.quiver(*origin.T, x_axis[:, 0], x_axis[:, 1], color="red", angles="uv")
    ax.quiver(*origin.T, y_axis[:, 0], y_axis[:, 1], color="green", angles="uv")

    ax.plot(*origin.T, linestyle=traj_linestyle, color=traj_color, label=traj_label)
