from typing import Literal, List, Tuple, Union, Callable, Dict
import os
import torch
import einops


def dict_apply(x: Dict[str, torch.Tensor], f: Callable) -> Dict:
    """Apply a function to all values in a dictionary."""
    result = {}
    for key, value in x.items():
        if isinstance(value, dict):
            result[key] = dict_apply(value, f)
        else:
            result[key] = f(value)
    return result
...
def patches_to_image(x: torch.Tensor, patch_size: int, image_size: Tuple[int, int]) -> torch.Tensor:
    """
