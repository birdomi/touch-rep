...
class SinusoidalEmbed(nn.Module):
    ...
    def __init__(
        ...
        assert len(size) < 4, "Sinusoidal position embeddings only support 1D, 2D and 3D grids."
        ...
        if logspace:
            frequency_bands = torch.stack(
                [
                    torch.logspace(0.0, -math.log2(resolution / 2.0), self.num_bands + 1, base=2)[:-1]
            ...
            frequency_bands = torch.stack(
                [torch.linspace(0, 1.0, steps=self.num_bands + 1)[:-1] for _ in range(len(size))],
                dim=0,
            )
            frequency_bands = resolution**-frequency_bands
        self.register_buffer("offset", torch.tensor(self.get_offset_coord(offset, patch_grid_size)))
        self.register_buffer("frequency_bands", frequency_bands)
        self.register_buffer("cached_encoding", None, persistent=False)

    def get_offset_coord(self, offset: int, size: List[int]):
        strides = [np.prod(size[n:]) for n in range(1, len(size))] + [1]
        dims = [offset] + size[1:]
        return [offset // stride % dim if offset != 0 else 0 for stride, dim in zip(strides, dims)]
    ...
    def forward(
        ...
        offset = self.get_buffer("offset")
        grid = create_ndgrid(self.patches_resolution, device=device, normalized_coords=normalized_coords)
        grid = grid + offset
        if grid.dim() < 2:
            grid = grid[..., None]
...
class PatchDecoder(nn.Module):
    def __init__(
        ...
        """
        Each out_feature has its own Linear
        """
        super().__init__()
        self.weight = nn.Parameter(torch.zeros(size, out_features, in_features))
    ...
    def reset_parameters(self) -> None:
        _, in_features, _ = self.weight.shape
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        output = input.transpose(0, 1) @ self.weight.transpose(-1, -2)
