#!/usr/bin/env bash
# Object-wise 3:1 train/val runner for BrainCo vision-only grasp prediction.
#
# Runs four leave-one-object-out experiments:
#   train = three objects, val = one held-out object
#
# Usage:
#   bash scripts/run_vision_objectwise_grasp.sh
#   bash scripts/run_vision_objectwise_grasp.sh brainco/task/grasp_prediction/resnet18_combined
#   bash scripts/run_vision_objectwise_grasp.sh -- trainer.max_epochs=50
...
EXPERIMENT="brainco/task/grasp_prediction/resnet18_combined"
EXTRA_OVERRIDES=()
...
DATASET_TARGET="tactile_ssl.data.brainco_grasp_vision_dataset.BraincoGraspVisionDataset"

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
EXP_NAME="${EXPERIMENT##*/}"
LOG_DIR="${SCRIPT_DIR}/logs/vision_objectwise_${EXP_NAME}_${TIMESTAMP}"
SUMMARY_CSV="${SCRIPT_DIR}/results_vision_objectwise_${EXP_NAME}_${TIMESTAMP}.csv"
SUMMARY_TXT="${SCRIPT_DIR}/results_vision_objectwise_${EXP_NAME}_${TIMESTAMP}.txt"
...
echo "Object-wise vision grasp experiments"
...
{
    echo "Object-wise Vision Grasp Results (${TIMESTAMP})"
    echo "Experiment: ${EXPERIMENT}"
    echo ""
...
for val_obj in "${OBJECTS[@]}"; do
    ...
    val_dataset_override="data.val_dataset={_target_:${DATASET_TARGET},config:{window_time:\${data.window_time},window_overlap:\${data.window_overlap},interpolating_freq:\${data.interpolating_freq},crop_mode:\${data.crop_mode},crop_size:\${data.crop_size},data_roots:[{data_path:${DATA_ROOT},label_dirs:{${val_label_dirs}}}]},data_path:${DATA_ROOT},num_frames_per_sample:\${data.num_frames_per_sample},img_size:\${data.img_size},augment:false}"
    run_name="vision_grasp_objectwise_${EXP_NAME}_val_${val_obj}"
    ...
    cmd=(
        python train_task_brainco_vision.py
        "+experiment=${EXPERIMENT}"
        "${train_roots_override}"
        "${train_data_path_override}"
        "${val_dataset_override}"
        "experiment_name=${run_name}"
        "wandb.group=vision_grasp_objectwise_${EXP_NAME}"
        "wandb.tags=[brainco,vision,resnet18,grasp_prediction,objectwise,val_${val_obj}]"
        "all_split=false"
        "${EXTRA_OVERRIDES[@]}"
    )

    if HYDRA_FULL_ERROR=1 "${cmd[@]}" 2>&1 | tee "${log_file}"; then
        echo "[OK] val=${val_obj}"
    else
