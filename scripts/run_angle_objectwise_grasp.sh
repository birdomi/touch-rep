#!/usr/bin/env bash
# Object-wise 3:1 train/val runner for BrainCo angle grasp prediction.
#
# Runs four leave-one-object-out experiments:
#   train = three objects, val = one held-out object
#
# Usage:
#   bash scripts/run_angle_objectwise_grasp.sh
#   bash scripts/run_angle_objectwise_grasp.sh brainco/ours_vectors/task/grasp_prediction/dinov2_multi_scratch_combined
#   bash scripts/run_angle_objectwise_grasp.sh brainco/ours_vectors/task/grasp_prediction/dinov2_multi_combined -- task.model_task._target_=tactile_ssl.downstream_task.MeanPoolProbe
...
EXPERIMENT="brainco/ours_vectors/task/grasp_prediction/dinov2_multi_combined"
EXTRA_OVERRIDES=()
...
EXP_NAME="${EXPERIMENT##*/}"
LOG_DIR="${SCRIPT_DIR}/logs/objectwise_${EXP_NAME}_${TIMESTAMP}"
SUMMARY_CSV="${SCRIPT_DIR}/results_angle_objectwise_${EXP_NAME}_${TIMESTAMP}.csv"
SUMMARY_TXT="${SCRIPT_DIR}/results_angle_objectwise_${EXP_NAME}_${TIMESTAMP}.txt"
...
echo "Object-wise angle grasp experiments"
...
{
    echo "Object-wise Angle Grasp Results (${TIMESTAMP})"
    echo "Experiment: ${EXPERIMENT}"
    echo ""
...
for val_obj in "${OBJECTS[@]}"; do
    ...
    run_name="angle_grasp_objectwise_${EXP_NAME}_val_${val_obj}"
    ...
    cmd=(
        ...
        "wandb.group=angle_grasp_objectwise_${EXP_NAME}"
        "wandb.tags=[angle,grasp_prediction,objectwise,val_${val_obj}]"
        "all_split=false"
        "${EXTRA_OVERRIDES[@]}"
    )
