"""test_visualization_hot3d.py — BrainCo vs HOT3D visualization.

Row 0: BrainCo  (local hand frame)
Row 1: HOT3D    (local hand frame)
Row 2: BrainCo  (global vroot frame — wrist pos + rotation axes)
Row 3: HOT3D    (global vroot frame — wrist pos + rotation axes)

Usage:
    python test_visualization_hot3d.py \
        --brainco_ep    dataset/brainco/pretraining/pot/episode_0000 \
        --brainco_urdf  dataset/brainco/urdf \
        --hot3d_root    pretraining_dataset/hot3d \
        --out           vis/compare_hot3d.png
"""
import argparse
import os
...
def _collect_image_files(rgb_dir: Path) -> list:
...
def _find_brainco_rgb_files(ep_path: str):
    ep = Path(ep_path)
...
def _load_brainco_rgb(ep_path: str, abs_frame: int, total_frames: int):
...
def extract_hot3d(data_root: str, seq_idx: int = -1, frame_idx: int = 0,
    ...
    assert len(dataset) > 0, f"No data found in {data_root}"

    win_idx = len(dataset) // 2 if seq_idx < 0 else min(seq_idx, len(dataset) - 1)
    sample  = dataset[win_idx]
    frame_idx = 0#min(frame_idx, sample["sensor"].shape[0] - 1)

    w_seq_idx, start = dataset.windows[win_idx]
    pkl_name  = dataset._seq_paths[w_seq_idx].name
    ...
    print(f"  HOT3D: dataset={len(dataset)} windows, win={win_idx}, "
          f"seq={w_seq_idx}({pkl_name}), abs_frame={abs_frame}")
    ...
    return dict(
        ...
        wrist_poses=wrist_p,
        sk_lines=_MP_LINES,
        info=f"HOT3D  {pkl_name}  frame={abs_frame}",
    )
...
def _render_and_save(bc, hot3d, out_path: str):
    ...
    nrows = 4   # local BC, local HOT3D, global BC, global HOT3D

    fig = plt.figure(figsize=(4.8 * ncols, 5.5 * nrows), constrained_layout=True)
    fig.suptitle(
        "BrainCo / HOT3D  —  Local (rows 0-1) / Global vroot (rows 2-3)",
        fontsize=11,
    )

    def make_row(row, with_rgb: bool = False, rgb=None, rgb_title: str = "RGB"):
        if with_rgb:
    ...
    # Local rows
    visualize_local(*make_row(0, with_rgb=True, rgb=bc_rgb,
                              rgb_title=f"BrainCo RGB  f{bc['abs_frame']}"), bc)
    visualize_local(*make_row(1), hot3d)
    # Global rows
    visualize_global(*make_row(2, with_rgb=True, rgb=bc_rgb,
                               rgb_title=f"BrainCo RGB  f{bc['abs_frame']}"), bc)
    visualize_global(*make_row(3), hot3d)
...
def main():
    ...
    parser.add_argument("--brainco_frame", type=int, default=0,
                        help="frame within window (-1=middle)")
    parser.add_argument("--frame_step",    type=int, default=100,
                        help="BrainCo sample index step for multi-frame output")
    parser.add_argument("--hot3d_root",    default="pretraining_dataset/hot3d")
    parser.add_argument("--hot3d_seq",     type=int, default=-1,
                        help="HOT3D window index (-1=middle)")
    parser.add_argument("--hot3d_frame",   type=int, default=0,
                        help="HOT3D frame within window")
    parser.add_argument("--window_size",   type=int, default=3)
    parser.add_argument("--out",           default="vis/compare_hot3d.png")
    args = parser.parse_args()

    out_path = Path(args.out)
    os.makedirs(out_path.parent, exist_ok=True)

    # Build BrainCo dataset once to know total sample count
    config = OmegaConf.create({
        "window_time": 0.15,
        "window_overlap": 0.0,
        "interpolating_freq": 100,
    ...
    print("Loading HOT3D sample...")
    hot3d = extract_hot3d(args.hot3d_root,
                          seq_idx=args.hot3d_seq,
                          frame_idx=args.hot3d_frame,
    ...
    for sample_idx in sample_indices:
        ...
        _render_and_save(bc, hot3d, str(out_file))
